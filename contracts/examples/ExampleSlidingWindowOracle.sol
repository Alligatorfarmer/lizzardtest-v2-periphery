...
import '../libraries/LizzardV2TestOracleLibrary.sol';
...
contract ExampleSlidingWindowOracle {
    ...
    function update(address tokenA, address tokenB) external {
        ...
        if (timeElapsed > periodSize) {
            (uint price0Cumulative, uint price1Cumulative,) = LizzardV2TestOracleLibrary.currentCumulativePrices(pair);
    ...
    function consult(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut) {
        address pair = LizzardV2TestLibrary.pairFor(factory, tokenIn, tokenOut);
        ...
        (uint price0Cumulative, uint price1Cumulative,) = LizzardV2TestOracleLibrary.currentCumulativePrices(pair);
        (address token0,) = LizzardV2TestLibrary.sortTokens(tokenIn, tokenOut);
